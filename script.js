// v6 client: adds Status (alive/dead) and Deputy (2nd in Command).
// Deputies get a crown and are included in "leaders only". Dead players show a red X overlay.
...
function populateFactionControls() {
  ...
  modalAllegiances.innerHTML = FACTIONS.filter(f => f !== "None / Unassigned").map(f => `<option value="${f}">${f}</option>`).join("");
}
